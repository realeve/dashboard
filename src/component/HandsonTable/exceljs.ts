...
const initWorkSheet = (config: Config) => {
  ...
  const totalLevel = getHeadLevel(config.columns);
...
// 根据一组合并单元格获取需要合并的起始列
const getRowStartAndEnd = (column, { min, max }) => {
  if (column.dataIndex) {
    min = Number(column.dataIndex.replace('col', ''));
    return { min, max: min };
  }
  if (column.children) {
    const res = getRowStartAndEnd(column.children, { min, max });
    min = Math.min(res.min, min);
    max = Math.max(res.max, max);
  } else {
    const first = R.head(column);
    const last = R.last(column);
    if (first.children) {
...
// 处理原始列信息，用于数据合并指引
const handleSrcColumn = (src, totalLevel, parentRow = 1) =>
  src.map((item) => {
    ...
    // 获取合并列信息
    const { min, max } = getRowStartAndEnd([item], { min: Infinity, max: 0 });
    const rowInfo = R.pick(['rowId', 'rowEndId', 'title', 'children', 'dataIndex'])(item);
    item = { min: min + 1, max: max + 1, ...rowInfo };
    return item;
  });

const mergeRowWithWorksheet = (setting, worksheet, extra) => {
  const offset = extra.rows ? 2 : 0;
  ...
  setting.forEach((config) => {
    ...
    const row = worksheet.getRow(config.rowId + offset);
    const cell = row.getCell(config.min);
...
const mergeRowInfo = (worksheet, config) => {
  const totalLevel = getHeadLevel(config.columns);
  // 获取各行列的合并结果;
  const mergeSetting = handleSrcColumn(R.clone(config.columns), totalLevel);
  mergeRowWithWorksheet(mergeSetting, worksheet, config.extra);

  // 处理前两行表头
  if (config.extra.rows) {
    const maxColumns = config.header.length;
    // 全并首行，第二行
    mergeRowByIdx(worksheet, 1, maxColumns, company + config.filename.replace('.xlsx', ''));
    const text = getTableExtraLabel(config.extra).join('          ');
...
const handleFormula = (worksheet, config: Config) => {
  ...
  let rowValues = [];
  const cols = config.params.stat_sum || [];
  if (cols.length > 0) {
    rowValues[0] = '汇总：求和';
    cols.forEach((i) => {
      const col = alpha[i - 1];
  ...
  const avgCol = config.params.stat_avg || [];
  ...
  for (let i = startRow; i <= worksheet.rowCount; i++) {
    ...
    for (let j: number = 1; j <= worksheet.columnCount; j++) {
      ...
      if (isInt(cell.value)) {
        ...
        const decimal = params.decimal || '2';
        const append = '0'.padStart(Number(decimal), '0');
        // 单元格计数默认小数位
        cell.numFmt = `#,##0.${append}`;
        cell.value = Number(cell);
      } else if (isTime(cell.value)) {
        // eslint-disable-next-line
...
const createWorkBook = (config: Config) => {
  const { worksheet, workbook } = initWorkSheet(config);
  const { params } = config;
  ...
  const headLevel = needHandleMerge ? getHeadLevel(config.columns) : 1;
  if (needHandleMerge) {
    // 复制一列数据到第2列作为表头内容
    const newRow = R.map(R.prop('header'))(columns);
    const headRow = new Array(headLevel - 1).fill(0).map(() => newRow);

    // 如果额外增加两行，需要插入两行空数据，否则前两条数据会失效
    // 2019-09-10
    const extra = config?.extra?.rows > 0 ? [...headRow, [], []] : headRow;
  ...
  const mergeCol = (start, end, key) => {
    ...
    const offset = needHandleMerge ? 3 : 2;
  ...
  if (params.mergev.length) {
    ...
    mergev.forEach((key) => {
      // 获取数据项
      const rows = lib.getDataByIdx({ key, data: config.body });
      let start = 0;
  ...
  //   item.map(td => {
  //     if (lib.isNumOrFloat(td)) {
  //       td = Number(td);
  //     } else if (lib.isTime(td)) {
  //       td = td.split('.')[0];
  //     }
  //     return td;
  //   })
...
const handleFilename: (name: string, param: DstConfig) => string = (filename, param) => {
  ...
  if (param.suffix) {
    const arr: string[] = filename.split('.xlsx');
...
export const save: (config: SaveSetting) => void = ({ params, ...config }) => {
  ...
  const workbook = createWorkBook(newConfig);
...
export const handleMergeV = (data: (string | number)[][], mergev: number[]) => {
  ...
  mergev.forEach((key) => {
    // 获取数据项
    const rows = lib.getDataByIdx({ key, data });
    let start = 0;
    for (let end = 1; end < rows.length; end++) {
